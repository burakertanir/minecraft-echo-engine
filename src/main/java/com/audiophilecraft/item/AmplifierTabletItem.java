...
public class AmplifierTabletItem extends Item {
        ...
        if (!world.isClient) {
            user.openHandledScreen(new ExtendedScreenHandlerFactory() {
                ...
                public ScreenHandler createMenu(int syncId, PlayerInventory playerInventory, PlayerEntity player) {
                    return new AmplifierScreenHandler(syncId, playerInventory,
                            ScreenHandlerContext.create(world, player.getBlockPos()));
                }
    ...
    /** Find all speakers in range of a position (used by server-side play commands) */
    public static List<BlockPos> scanForSpeakers(BlockPos playerPos) {
        return SpeakerRegistry.findSpeakersInRange(playerPos, SCAN_RADIUS);
    }
}
