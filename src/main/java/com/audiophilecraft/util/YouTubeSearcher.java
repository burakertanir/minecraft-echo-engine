...
public class YouTubeSearcher {
    ...
    public static List<SearchResult> search(String query) {
        ...
        try {
            ...
            while (results.size() < 5) {
                ...
                String title = extractBoundedJsonValue(json, "\"title\":{\"runs\":[{\"text\":\"", "\"", rendererIdx, maxBoundary);
                if (title == null) continue;
                
                String channel = extractBoundedJsonValue(json, "\"shortBylineText\":{\"runs\":[{\"text\":\"", "\"", rendererIdx, maxBoundary);
                if (channel == null) channel = extractBoundedJsonValue(json, "\"longBylineText\":{\"runs\":[{\"text\":\"", "\"", rendererIdx, maxBoundary);
                if (channel == null) channel = "YouTube";
                
                String duration = extractBoundedJsonValue(json, "\"simpleText\":\"", "\"", rendererIdx, maxBoundary);
                if (duration == null) {
                     // Fallback check
                     duration = extractBoundedJsonValue(json, "\"lengthText\":{\"accessibility\":{\"accessibilityData\":{\"label\":\"", "\"", rendererIdx, maxBoundary);
                     if (duration == null) duration = "";
                }
                
                results.add(new SearchResult(videoId, cleanJsonString(title), cleanJsonString(channel), cleanJsonString(duration)));
            }
    ...
    private static String extractBoundedJsonValue(String json, String startKey, String endKey, int startSearchFrom, int maxBoundary) {
