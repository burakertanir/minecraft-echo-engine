...
public class AmplifierScreen extends HandledScreen<AmplifierScreenHandler> {
    ...
    private static volatile boolean isFetchingThumbnail =
            false; // volatile: written by background thread, read by render thread
    ...
    private volatile java.util.List<com.audiophilecraft.util.YouTubeSearcher.SearchResult> searchResults =
            new java.util.ArrayList<>(); // volatile: assigned from background thread, iterated on render thread
    ...
    protected void init() {
        ...
        // Ensure the active session on the client matches the player's tablet UUID
        if (client != null && client.player != null) {
            com.audiophilecraft.sound.AudioEngine.getInstance().ensureActiveSession(client.player.getUuid());
        }
        ...
        // acts as padding.
        urlField = new TextFieldWidget(textRenderer, x + 44, clusterY, backgroundWidth - 88, 20, Text.literal("URL")) {
            @Override
            public void renderButton(DrawContext context, int mouseX, int mouseY, float delta) {
                ...
                if (protectiveFillAlpha != 0) {
                    context.fill(
                            getX() - 4, getY(), getX() + width + 4, getY() + height, protectiveFillAlpha | 0x000000);
                }
            ...
            if (extractYouTubeId(trimmed) != null || trimmed.startsWith("http")) {
                ...
                if (activeDisplayTitle.isEmpty()) {
                    Thread t = new Thread(() -> {
                        ...
                        java.util.List<com.audiophilecraft.util.YouTubeSearcher.SearchResult> res =
                                com.audiophilecraft.util.YouTubeSearcher.search(query);
        ...
        inputGainSlider = new InputGainSlider(
                x + 40,
                clusterY + 130,
                backgroundWidth - 80,
                20,
                Text.literal("Input Gain: " + initialPercentage + "%"),
                initialGainVal);
        ...
        powerSlider = new PowerSlider(
                ...
                backgroundWidth - 80,
                20,
                Text.literal("Power: " + String.format("%.1f", currentPower)),
                initialSliderValue);
        ...
        for (int col = 0; col < 3; col++) {
            ...
            for (int r = 0; r < 6; r++) {
                ...
                if (r > 0) {
                    ...
                    QSliderWidget qSlider =
                            new QSliderWidget(cx + 10 + colWidth - 40, startY + r * 22 + 15, 20, 14, types[col], r);
        ...
        midButton =
                new TransparentButton(x + (backgroundWidth / 2) - 60, midSideY, 50, 20, getMidButtonText(), button -> {
                    boolean current =
                            com.audiophilecraft.sound.AudioEngine.getInstance().isMidMuted();
                    com.audiophilecraft.sound.AudioEngine.getInstance().setMidMuted(!current);
                    button.setMessage(getMidButtonText());
                });
        sideButton =
                new TransparentButton(x + (backgroundWidth / 2) + 10, midSideY, 50, 20, getSideButtonText(), button -> {
                    boolean current =
                            com.audiophilecraft.sound.AudioEngine.getInstance().isSideMuted();
    ...
    public void updateSpeakerPower(float power) {
        if (System.currentTimeMillis() - lastInteractionTime < 500) return; // Block slider snap-backs
        currentPower = power;
        if (powerSlider != null) {
    ...
    private String extractYouTubeId(String url) {
        ...
        java.util.regex.Matcher m = java.util.regex.Pattern.compile(
                        "(?<=v=|v\\/|vi=|vi\\/|youtu.be\\/|\\/v\\/|embed\\/)([a-zA-Z0-9_-]{11})")
                .matcher(url);
            ...
            try {
                if (videoId != null) {
                    java.awt.image.BufferedImage img =
                            com.audiophilecraft.client.util.YouTubeThumbnailCache.loadAndCache(videoId);
    ...
    private int[] getDominantColors(java.awt.image.BufferedImage img) {
        ...
        for (int i = 0; i < 4096; i++) {
            ...
            if (dr * dr + dg * dg + db * db < 16) continue; // Ensure distinct color
            if (bins[i] > maxCount2) {
                maxCount2 = bins[i];
                secondaryBin = i;
            }
        ...
        int maxCount3 = 0;
        for (int i = 0; i < 4096; i++) {
            if (i == primaryBin || i == secondaryBin) continue;
            int dr1 = (i >> 8) - (primaryBin >> 8),
                    dg1 = ((i >> 4) & 0xF) - ((primaryBin >> 4) & 0xF),
            ...
            if (dr1 * dr1 + dg1 * dg1 + db1 * db1 < 16 || dr2 * dr2 + dg2 * dg2 + db2 * db2 < 16) continue;
            if (bins[i] > maxCount3) {
                maxCount3 = bins[i];
                tertiaryBin = i;
            }
        ...
        if (secondaryBin == -1) secondaryBin = primaryBin;
        if (tertiaryBin == -1) tertiaryBin = secondaryBin;
        int c1 = primaryBin != -1 && bins[primaryBin] > 0
                ? (int) (sumR[primaryBin] / bins[primaryBin]) << 16
                        | (int) (sumG[primaryBin] / bins[primaryBin]) << 8
                        | (int) (sumB[primaryBin] / bins[primaryBin])
                : 0x222222;
        int c2 = secondaryBin != -1 && bins[secondaryBin] > 0
                ? (int) (sumR[secondaryBin] / bins[secondaryBin]) << 16
                        | (int) (sumG[secondaryBin] / bins[secondaryBin]) << 8
                        | (int) (sumB[secondaryBin] / bins[secondaryBin])
                : c1;
        int c3 = tertiaryBin != -1 && bins[tertiaryBin] > 0
                ? (int) (sumR[tertiaryBin] / bins[tertiaryBin]) << 16
                        | (int) (sumG[tertiaryBin] / bins[tertiaryBin]) << 8
                        | (int) (sumB[tertiaryBin] / bins[tertiaryBin])
                : c2;
        ...
        if (urlField != null) {
            ...
            if (isDropdownOpen && currentText.length() >= 3 && (now - lastTypedTime) > 500) {
                if (!currentText.equals(lastSearchedQuery) && !isSearching) {
                    ...
                    Thread t = new Thread(() -> {
                        java.util.List<com.audiophilecraft.util.YouTubeSearcher.SearchResult> res =
                                com.audiophilecraft.util.YouTubeSearcher.search(currentText);
                        searchResults = res;
                        isSearching = false;
                    });
                    t.setDaemon(true);
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (isMapOpen) {
            int startX = (width - backgroundWidth) / 2;
            int startY = (height - backgroundHeight) / 2;
            java.util.List<net.minecraft.util.math.Vec3d> pc =
                    com.audiophilecraft.sound.AdvancedAcousticScanner.getLastPointCloud();
            if (pc != null && !pc.isEmpty()) {
                // Background fill for the map area
                context.fill(
                        startX + 10,
                        startY + 30,
                        startX + backgroundWidth - 10,
                        startY + backgroundHeight - 30,
                        0xCC000000);
                java.util.List<net.minecraft.util.math.BlockPos> speakers =
                        com.audiophilecraft.sound.AdvancedAcousticScanner.getLastSpeakers();
                ...
                com.audiophilecraft.sound.AdvancedAcousticScanner.VenuePreset preset =
                        com.audiophilecraft.sound.AudioEngine.getInstance().getVenuePreset();
                if (preset != null && preset.tierName != null) {
                    ...
                    context.drawText(
                            textRenderer,
                            tierStr,
                            startX + backgroundWidth - 15 - textW,
                            startY + 35,
                            0xFFFFDD00,
                            false);
                }
                // Draw Sabine Volume in the bottom right
                com.audiophilecraft.sound.AdvancedAcousticScanner.VenueDescriptor desc =
                        com.audiophilecraft.sound.AudioEngine.getInstance().getStoredVenueDescriptor();
                if (desc != null) {
                    ...
                    context.drawText(
                            textRenderer,
                            volStr,
                            startX + backgroundWidth - 15 - volW,
                            startY + backgroundHeight - 45,
                            0xFF00FFFF,
                            false);
                }
            } else {
            ...
            if (currentParsedVideoId != null) {
                ...
                int thumbY = panelY;
                Identifier thumbId =
                        com.audiophilecraft.client.util.YouTubeThumbnailCache.getIdentifier(currentParsedVideoId);
                if (thumbId != null) {
            ...
            if (isDropdownOpen && urlField != null && urlField.isFocused()) {
                ...
                if (isSearching) {
                    ...
                    context.fill(
                            dropX + dropWidth - 1, dropY, dropX + dropWidth, dropY + 20, currentAdaptiveThemeColor);
        ...
        if ((isSearching || isFetchingThumbnail) && urlField != null) {
            ...
            // Faded track
            context.fill(
                    rightX, barY, rightX + barWidth, barY + 1, (currentAdaptiveThemeColor & 0xFFFFFF) | 0x44000000);
            // Sliding head
    ...
    private class SeekBarWidget extends net.minecraft.client.gui.widget.SliderWidget {
        ...
        public void onRelease(double mouseX, double mouseY) {
            ...
            if (total > 0) {
                ...
                net.fabricmc.fabric.api.client.networking.v1.ClientPlayNetworking.send(
                        com.audiophilecraft.network.ModMessages.C2S_SEEK_TRACK, buf);
            }
            this.setFocused(false);
        }
        ...
        public void autoUpdate() {
            if (AmplifierScreen.this.getFocused() != this) {
                double total =
                        com.audiophilecraft.sound.AudioEngine.getInstance().getTotalPlaybackDuration();
                double current =
                        com.audiophilecraft.sound.AudioEngine.getInstance().getCurrentPlaybackTime();
                if (total > 0) {
        ...
        public void renderButton(net.minecraft.client.gui.DrawContext context, int mouseX, int mouseY, float delta) {
            ...
            if (isHovered() || AmplifierScreen.this.getFocused() == this) {
                context.fill(
                        knobX - 2, getY() - 2, knobX + knobWidth + 2, getY() + height + 2, currentAdaptiveThemeColor);
                context.fill(
                        knobX - 2,
                        getY() - 2,
                        knobX + knobWidth + 2,
    ...
    private class MixerSliderWidget extends net.minecraft.client.gui.widget.SliderWidget {
        ...
        public MixerSliderWidget(int x, int y, int width, int height, String speakerType, int typeIndex) {
            ...
            if (typeIndex == 0) {
                float linearGain =
                        com.audiophilecraft.sound.AudioEngine.getInstance().getMixerGain(speakerType);
        ...
        protected void applyValue() {
            if (typeIndex == 0) {
                ...
                java.util.UUID myUUID = net.minecraft.client.MinecraftClient.getInstance().player != null
                        ? net.minecraft.client.MinecraftClient.getInstance()
                                .player
                                .getUuid()
                        : java.util.UUID.randomUUID();
                com.audiophilecraft.sound.AudioEngine.getInstance()
                        .setMixerGainForSession(myUUID, speakerType, actualGain);
                // Send to server for multiplayer sync
                net.minecraft.network.PacketByteBuf gainBuf =
                        net.fabricmc.fabric.api.networking.v1.PacketByteBufs.create();
                gainBuf.writeString(speakerType);
                gainBuf.writeFloat(actualGain);
                net.fabricmc.fabric.api.client.networking.v1.ClientPlayNetworking.send(
                        com.audiophilecraft.network.ModMessages.C2S_UPDATE_MIXER_GAIN, gainBuf);
            } else {
                float db = (float) (this.value * 24.0 - 12.0);
                ...
                java.util.UUID myUUID = net.minecraft.client.MinecraftClient.getInstance().player != null
                        ...
                        : java.util.UUID.randomUUID();
                com.audiophilecraft.sound.AudioEngine.getInstance()
                        .setEqDbForSession(myUUID, speakerType, typeIndex - 1, db);
                // Send to server for multiplayer sync
                net.minecraft.network.PacketByteBuf eqBuf =
                        net.fabricmc.fabric.api.networking.v1.PacketByteBufs.create();
                eqBuf.writeString(speakerType);
                eqBuf.writeInt(typeIndex - 1);
                eqBuf.writeFloat(db);
                net.fabricmc.fabric.api.client.networking.v1.ClientPlayNetworking.send(
                        com.audiophilecraft.network.ModMessages.C2S_UPDATE_EQ, eqBuf);
            }
        }
    ...
    private class QSliderWidget extends net.minecraft.client.gui.widget.SliderWidget {
        ...
        protected void applyValue() {
            ...
            java.util.UUID myUUID = net.minecraft.client.MinecraftClient.getInstance().player != null
                    ? net.minecraft.client.MinecraftClient.getInstance().player.getUuid()
                    : java.util.UUID.randomUUID();
            com.audiophilecraft.sound.AudioEngine.getInstance().setEqQForSession(myUUID, speakerType, typeIndex - 1, q);
            // Send to server for multiplayer sync
            ...
            qBuf.writeFloat(q);
            net.fabricmc.fabric.api.client.networking.v1.ClientPlayNetworking.send(
                    com.audiophilecraft.network.ModMessages.C2S_UPDATE_EQ_Q, qBuf);
        }
