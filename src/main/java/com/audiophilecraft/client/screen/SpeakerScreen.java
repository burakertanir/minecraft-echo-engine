...
public class SpeakerScreen extends HandledScreen<SpeakerScreenHandler> {
    ...
    protected void init() {
        ...
        if (pos != null && MinecraftClient.getInstance().world != null) {
            net.minecraft.block.Block block = MinecraftClient.getInstance().world.getBlockState(pos).getBlock();
        ...
        shiftSlider = new ShiftSlider(x + 10, y + 30, 156, 20,
                Text.literal("Sample Shift: " + handler.getSampleShift() + " ms"), initialShift);
        ...
        if (allowsTilt) {
            ...
            tiltSlider = new TiltSlider(x + 10, y + 58, 156, 20,
                    Text.literal("Vertical Tilt: " + handler.getVerticalTiltDeg() + "°"), initialTilt);
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        RenderSystem.setShaderTexture(0, new Identifier("audiophilecraft", "textures/gui/amplifier_gui.png"));
        int x = (width - backgroundWidth) / 2;
        int y = (height - backgroundHeight) / 2;
        context.drawTexture(new Identifier("audiophilecraft", "textures/gui/amplifier_gui.png"), x, y, 0, 0,
    ...
    private class TiltSlider extends SliderWidget {
        ...
        protected void updateMessage() {
            ...
            this.setMessage(Text.literal("Vertical Tilt: " + deg + "° " + (deg < 0 ? "(Aşağı)" : deg > 0 ? "(Yukarı)" : "(Düz)")));
        }
